...
def _get_param_type(
    ...
    is_type = isinstance(param_hint, type)
    ...
    elif is_type and issubclass(param_hint, (int, float, bytes)):
        return _ParamArgs.single, param_hint
    elif is_type and issubclass(param_hint, bool):
        return _ParamArgs.flag, None
    elif is_type and issubclass(param_hint, list):
        return _ParamArgs.multiple, None
    elif is_type and issubclass(param_hint, datetime.datetime):
        return _ParamArgs.single, click.DateTime()
...
class _CommandBuilder:
    ...
    def _add_parameters(self):
        ...
        for name, param in self.sig.parameters.items():
            ...
            decorator = _get_param_decorator(
                param,
                param_args,
                param_type,
                param_doc,
    ...
    def _finalise(self):
        ...
        vp_params = list(p for p in params if p.kind == p.VAR_POSITIONAL)
        if vp_params:
